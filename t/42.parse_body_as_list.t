...
use Test::More tests => 207;    # toooooooooooooooo much ^^;
use Test::Exception;
use Test::Warn;
...
# ================================================================
# normal
# [ 5subtests * 1kind * 12kinds * 3versions = 180 tests ]
# ----------------------------------------------------------------
foreach my $version (@Versions) {
    my %version_definition = ( version => $version );

    foreach my $test_case (@{ $Parser{normal}{array_ref} }) {   # 12kinds
        my $parsing_query = [
...
# [ 3subtests * 1kind * 3kinds * 3versions = 27 tests ]
# ----------------------------------------------------------------
foreach my $version (@Versions) {
    my %version_definition = ( version => $version );

    foreach my $test_case (@{ $Parser{warning}{array_ref} }) {
        my $parsing_query = [
            (split "\n", $Query{header}{$version}{'en/ja'}{has_column}),
            @{ $test_case->{query} }
        ];
